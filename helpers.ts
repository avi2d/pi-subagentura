...
export function scheduleJobCleanup(jobId: string, immediate = false, maxAge?: number): void {
  if (!immediate) {
    if (maxAge && maxAge > 0) {
      setTimeout(() => {
        jobRegistry.delete(jobId)
...
export function formatUsage(u: SubagentResult['usage'], model?: string): string {
  const parts: string[] = []
...
export async function startSubagentJob(
  ...
  const { task, persona, modelOverride, cwd, contextText, signal, onUpdate, defaultModel } = params
  ...
  const modelLabel = targetModel ? `${targetModel.provider}/${targetModel.id}` : undefined
