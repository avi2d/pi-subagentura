...
flowchart TB
  ...
  Coordinator -->|hidden compact manifest| Parent
```
...
- Default asynchronous parent delivery is one hidden `subagent-manifest`, attached to a natural human turn or sent as a safe triggered follow-up. TUI completion entries are a separate non-LLM channel.
...
- `src/completion-coordinator.ts` owns per-session completion records, consumption receipts, each/group readiness, and compact manifest claims.
...
- `completionPolicy="each"` makes records independently ready and coalescible. `completionPolicy="group"` waits for a sealed all-terminal group.
- The coordinator attaches one ready manifest to pending human input or triggers one follow-up only after safe idleness. Full output is not automatic by default.
...
| human `input` / `before_agent_start` | Give human input priority and attach any ready compact manifest to the natural turn                                 | `src/session-handlers.ts`, `completion-coordinator.ts`                |
| `agent_start`                        | Mark this parent as streaming                                                                                       | `src/session-handlers.ts`                                             |
| `agent_settled`                      | Clear streaming, seal groups, and dispatch remaining ready work after legacy flushes                                | `src/session-handlers.ts`, `completion-coordinator.ts`                |
...
`src/completion-coordinator.ts` is the default asynchronous completion broker.
...
`attachAsyncJobSettlement` prevents a cancelled record from being changed by late settlement. The coordinated default publishes one TUI-only terminal entry and one bounded job-result reference. Independent records coalesce while busy. Grouped records wait for parent settlement to seal membership and for every member—including errors and cancellations—to become terminal. Deprecated legacy fields map to coordinated `each`; only persisted pre-coordinator state retains bounded inject/pointer recovery behavior.
...
On completion, `notifyWorkflowCompletion` publishes one coordinated workflow record by default. The compact manifest points to `get_workflow_result`; successful terminal retrieval consumes pending automatic delivery. Workflow-owned process and in-process children are suppressed, so internal fan-out never wakes the parent per child. Explicit groups share the same parent-settlement seal and all-terminal barrier as other background work. Publication is owner-fenced and suppressed by owner cleanup.
...
| Coordinated completion entry | Completion coordinator                  | Human TUI / reconciliation      | Bounded terminal identity and references           | Parent session                              | No                                 | Exactly-once user notice                               |
| `subagent-manifest`          | Completion coordinator                  | Parent Pi session/agent         | Compact coalesced references and completion IDs    | After Pi session commit                     | Yes                                | Default parent-delivery claim                          |
...
| Parent completion/consumption entries   | Parent Pi session custom entries                                       |                    Yes |         Yes with session | Coordinator reconciliation             |
...
The following table inventories all 45 source files exactly once as modules.
...
|   8 | `src/delivery.ts`                            | Durable interactive intent queue, coordinated publication, validated legacy injection, and receipt reconciliation          | `artifact`, `completion-coordinator`, `notifications`, `session-scope`, `helpers`; type-only `interactive-tmux`                                                                                                                                                                |
...
|  25 | `src/session-handlers.ts`                    | Pi lifecycle adapter, sole poll interval, rehydrate, human-priority completion coordination, and shutdown                  | `artifact`, `artifact-poller`, `completion-coordinator`, `delivery`, `notifications`, `helpers`, `cancellation-snapshots`, `rehydrate`, `interactive-tmux`, `workflow-jobs`, `session-scope`, `interactive-supervisor-ui`                                                      |
...
|  43 | `src/completion-coordinator.ts`              | Per-session TUI entries, each/group readiness, manual consumption, human-priority manifest attachment, and continuation    | `session-scope`, `helpers`                                                                                                                                                                                                                                                     |
...
- [ ] Every terminal record appends one deterministic TUI completion entry that is excluded from LLM context.
- [ ] Independent records become ready immediately and coalesce while the parent is busy.
- [ ] Group membership is explicit, seals at parent settlement, and waits for all done/error/cancelled members.
...
- [ ] Successful terminal retrieval consumes matching pending automatic delivery.
