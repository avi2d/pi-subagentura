...
function renderSubagentCall(args: Record<string, unknown>, theme: Theme, label: string) {
...
function renderSubagentResult(
  ...
  if (isPartial) {
    const status = result.details?.subagentStatus as SubagentLiveStatus | undefined
 ...
 */
function renderAsyncSpawn(details: Record<string, unknown>, theme: Theme): Text {
...
const StatusParams = Type.Object({
  jobId: Type.String({
    description: 'Job ID returned by async subagent_with_context or subagent_isolated spawn',
  }),
})
...
function incrementInjectCount(): void {
  const g2 = typeof global !== 'undefined' ? global : globalThis
  g2.__piSubagenturaInjectCount = ((g2.__piSubagenturaInjectCount ?? 0) as number) + 1
}
...
function deliverNotification(jobState: JobState, result: SubagentResult): void {
  ...
  try {
    ...
    if (jobState.notifyOnComplete === 'inject') {
      ...
      ;(pi as any).sendUserMessage?.(result.output || '(sub-agent produced no output)', {
        deliverAs: 'followUp',
      })
...
function renderSubagentNotify(message: any, options: { expanded?: boolean }, theme: Theme): Text {
  ...
  if (!options.expanded) {
    ...
    const output = sanitizeOutput(
      ((details?.result as SubagentResult)?.output ?? '').slice(0, 500).replace(/\s+/g, ' ')
    )
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (params.async === true) {
        ...
        const { jobId, jobPromise, session, liveStatus } = await startSubagentJob({
          task: params.task,
        ...
        jobPromise.then(
          (result) => {
            ...
            const remaining = [...jobRegistry.values()].filter((j) => j.status === 'running').length
            if (remaining > 0) {
              try {
                ctx.ui.setStatus(
                  FOOTER_KEY,
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      ...
      if (job.status === 'cancelled') {
        return {
          content: [{ type: 'text', text: `Job ${params.jobId} was cancelled.` }],
    ...
    renderCall(args, theme) {
      ...
      const text =
        theme.fg('toolTitle', theme.bold('get_subagent_status ')) + theme.fg('accent', jobId)
      return new Text(text, 0, 0)
    },
    ...
    renderResult(result, options, theme, context) {
      ...
      if (details?.status === 'running') {
        ...
        return renderSubagentResult(result, { ...options, isPartial: true }, theme, context)
      }
      return renderSubagentResult(result, options, theme, context)
    },
  })
  ...
  pi.registerTool({
    ...
    renderCall(args, theme) {
      const jobId = String(args.jobId ?? '')
      const text =
        theme.fg('toolTitle', theme.bold('get_subagent_result ')) + theme.fg('accent', jobId)
      return new Text(text, 0, 0)
    },
  ...
  pi.registerTool({
    ...
    renderCall(args, theme) {
      const jobId = String(args.jobId ?? '')
      const text = theme.fg('toolTitle', theme.bold('cancel_subagent ')) + theme.fg('error', jobId)
      return new Text(text, 0, 0)
    },
    ...
    renderResult(result, _options, theme, _context) {
      ...
      const message = firstContent?.type === 'text' ? firstContent.text : `Job ${jobId} not found`
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      filter: Type.Optional(
        Type.String({
          description: 'Optional substring filter for provider or model name (case-insensitive)',
        })
      ),
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const models =
        params.authOnly !== false ? modelRegistry.getAvailable() : modelRegistry.getAll()
      ...
      const lines = filtered.map(
        (m) => `${m.provider}/${m.id}` + (m.name && m.name !== m.id ? `  (${m.name})` : '')
      )
  ...
  pi.registerTool({
    ...
    renderCall(_args, theme) {
      return new Text(theme.fg('toolTitle', theme.bold('prune_subagent_jobs')), 0, 0)
    },
    ...
    renderResult(result, _options, theme, _context) {
      ...
      const text =
        removed > 0
          ? theme.fg('success', `✓ Pruned ${removed} job${removed === 1 ? '' : 's'}`)
