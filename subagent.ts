...
export default function(pi: ExtensionAPI) {
   ...
   pi.registerTool({
      ...
      description: [
         "Spawn a subagent in an isolated tmux session with notification on completion.",
         "The user can attach to the tmux session to interact with the subagent directly.",
         "Attach with: tmux attach -t pi-subagentura:<jobId>",
         "When complete, the subagent sends a notification with results.",
      ].join("\n"),
      parameters: SpawnNotifyParams,

      async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
         debugLog("info", "tool_call", {
            toolName: "spawn_notify_subagent",
            toolCallId: _toolCallId,
         });
         ...
         try {
            ...
            return {
               content: [
                  {
                     type: "text",
                     text: `Failed to spawn subagent: ${(err as Error).message}`,
                  },
               ],
               isError: true,
      ...
      renderCall(args, theme) {
         ...
         const text =
            theme.fg("toolTitle", theme.bold("spawn_notify_subagent ")) +
            theme.fg("dim", `"${task}..."`);
         return new Text(text, 0, 0);
      },

      renderResult(result, _options, theme, _context) {
         const details = result.details as SpawnNotifyResult | undefined;
         if (!details) {
            return new Text(theme.fg("error", "Spawn failed"), 0, 0);
         }

         const lines = [
            theme.fg("success", "✓ Subagent spawned"),
