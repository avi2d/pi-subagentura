...
- **Option A — "shutting down" flag** — add `__piShuttingDown = true` before cancel, poller checks and returns early. Rejected because: adds runtime global check to every poll tick, introduces new state that must be managed/reset, and is unnecessary — clearing the registry achieves the same guarantee with zero new state.
- **Option B — Kill panes directly in shutdown** — inline `mux.killPane` calls in shutdown handler using snapshot. Rejected because: duplicates the `.cancelled` write + pane kill logic inside the shutdown handler; a dedicated export is cleaner and reusable.
- **Option C — Cancel before clear, no snapshot** — current order. Rejected because: a poll tick queued before `clearInterval` can observe registry entries mid-cancel and re-deliver notifications.

### Why Chosen
The snapshot-before-clear + new `cancelInteractiveSubagentByState` export is the minimum-change fix that eliminates the race entirely and preserves the registry-cleanup contract. The shutdown handler captures running state objects before clearing, so an in-flight poll tick finds an empty registry and its for-loop iterates over zero entries (no work, no notification). The cancel loop uses the new registry-bypass export so pane-kill and `.cancelled`-file write still succeed after the registry is cleared. The new export encapsulates a reusable primitive without changing the public API.

### Consequences
- **Positive:** Bug A eliminated without new global state; existing shutdown test continues to pass; race window completely closed; new `cancelInteractiveSubagentByState` export is a clean, reusable primitive that documents the registry-independent path explicitly.
- **Negative:** The cancel loop now iterates `interactiveSubagentRegistry` twice (once for snapshot, once for actual cancel) — O(N) overhead on shutdown only, acceptable. `cancelInteractiveSubagentByState` does not update `state.status` (acceptable — the registry is already cleared, so status bookkeeping is unnecessary for shutdown). Adds one new public export to `interactive-tmux.ts` (low blast radius — internal module).

### Follow-ups
- [ ] Monitor whether the `idle` footer-counting decision (Open Question B-1) should be revisited; if UX feedback says idle should not count, change the guard in Task 3 from `state.status === "running" || state.status === "idle"` to just `state.status === "running"`.
- [ ] Consider pruning `exited` sub-agents from registry after a grace period (per Open Question B-2) — out of scope for this fix.

### Approvals
- ✓ **Architect** at iteration 2 (final revision) — APPROVE. All issues resolved; new export design is clean; no new issues introduced.
- ✓ **Critic** at iteration 2 (final review) — APPROVE. Registry-bypass primitive correctly designed; shutdown ordering is correct; race window closed.

### Rejections
- ✗ **Architect iteration 0** (first draft): cancelled loop was a no-op (iterated already-cleared registry). Resolved in iteration 1.
- ✗ **Critic iteration 1**: calling `cancelInteractiveSubagent(id)` after `clear()` was a complete no-op (early-return on registry lookup). Resolved in iteration 2 by adding `cancelInteractiveSubagentByState`.
---
#MK:
#PW:## Open Questions
