/**
 * Subagent Status — the per-job state machine that drives widget labels and
 * stalled/recovered transitions.
 *
 * Mirrors pi-interactive-subagents (HazAT) `pi-extension/subagents/status.ts`,
 * adapted to our `SubagentActivityState` shape. Pure logic — no I/O.
 *
 *   createStatusState  → initial state per source
 *   observeStatus      → fold a new activity observation into the state
 *   classifyStatus     → compute the current StatusSnapshot (kind + timings)
 *   advanceStatusState → tick the state forward in time, return transition
 *   formatWidgetRightLabel → " active · read 2m " / " waiting 2m " / " stalled 4m "
 *   forceStatusAfterInterrupt → reset to "waiting" after Escape
 *
 * Sources:
 *   - "pi"     → activity files written by Pi-backed children (tmux)
 *   - "claude" → no activity file; falls back to a coarse "running" label
 */
...
/**
 * Build the right-side label for a widget line, matching pi-interactive-subagents'
 * `formatWidgetRightLabel`. Produces strings like:
 *   " starting… "      (no activity yet)
 *   " active · read 2m "
 *   " waiting 2m "
 *   " stalled 4m "
 *   " running 3m "
