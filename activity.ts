/**
 * Subagent Activity — per-job runtime snapshot written by child pi processes.
 *
 * Mirrors pi-interactive-subagents (HazAT) `pi-extension/subagents/activity.ts`.
 * The parent polls each running job's activity file to drive the widget status
 * state machine and fire stalled/recovered transitions.
 *
 * File format (`<sessionDir>/subagent-activity/<runningChildId>.json`):
 *   {
 *     "version": 1,
 *     "runningChildId": "tm-abc123",
 ...
 *     "toolActive": true,
 *     "activeScope": "tool",
 *     "activeSince": 1700000001000,
 *     "toolName": "read",
 *     ...
 *   }
 *
 * Atomic write: tmp + rename, so the parent never observes a half-written file.
 * Throttle: 500ms between flushes; immediate flush for state-changing events.
 * Auto-disable: 3 consecutive write failures → recorder becomes a no-op.
 */
...
/** Path to a job's activity file inside its session dir. */
export function getSubagentActivityFile(sessionDir: string, runningChildId: string): string {
	return join(sessionDir, "subagent-activity", `${runningChildId}.json`);
}
...
/**
 * Atomically read & validate a job's activity file.
 * Returns `{ ok: false, reason: "missing" | "invalid" | "wrong-id" }` on failure.
 */
...
/** Atomic write: tmp + rename so the parent never sees a half-written file. */
...
/**
 * Create a recorder that writes throttled snapshots to `activityFile`.
 *
 * If either `runningChildId` or `activityFile` is missing, returns a no-op
 * recorder. This is the safe default for in-process jobs that don't need
 * cross-process activity files.
 */
