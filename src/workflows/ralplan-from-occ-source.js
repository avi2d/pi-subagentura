// src/workflows/ralplan-from-occ-source.js
//
// Variation from ralplan-occ.js:
// - Consecutive-acceptance tracking: requires 2 consecutive ACCEPT/ACCEPT-WITH-RESERVATIONS
//   verdicts from the Critic (not just first-pass) before declaring consensus. This reduces
//   false positives from a single lenient review.
// - Token-budget-aware maxIterations: if the injected `budget` helper is available and reports
//   remaining tokens, the loop shortens to stay within budget rather than blindly iterating.
// - Backoff log signal between iterations: emits CONSECUTIVE_ACCEPTS_N to make iteration
//   progress visible without requiring a delay.
//
// All role personas are stripped of YAML frontmatter (name/description/model/level/
// disallowedTools) and the outer <Agent_Prompt>...</Agent_Prompt> wrapper — the raw
// structural sections are preserved verbatim.

export const meta = {
  name: "ralplan-from-occ-source",
  description:
    "RALPLAN consensus planning (OCC source variant): Analyst-spec -> Planner -> " +
    "Architect -> Critic loop with consecutive-acceptance gating. Produces an ADR-formatted " +
    "plan at .omc/plans/<planName>.md once consensus is reached.",
  whenToUse:
    ...
    "This variant requires 2 consecutive ACCEPT verdicts from the Critic before finalizing.",
  phases: [
    { title: "Spec", detail: "Optional Analyst -> .omc/plans/spec.md (skipped when args.specPath is provided)" },
    { title: "Planning", detail: "Planner / Architect / Critic loop with consecutive-acceptance gating (max args.maxIterations, default 5)" },
...
// ─────────────────────────────────────────────────────────────────────────────
// Verdict parsing — matches **VERDICT: APPROVE|REVISION NEEDED** and
// **VERDICT: REJECT|REVISE|ACCEPT-WITH-RESERVATIONS|ACCEPT**
// ─────────────────────────────────────────────────────────────────────────────
...
const DELIBERATE_SIGNALS = [
  ...
  "migration", "schema", "database", "production", "destroy",
  "delete", "authentication", "authorization", "authorized",
  "authorize", "authorizing", "compliance", "pii", "gdpr", "hipaa",
...
function validateArgs(args) {
  if (!args || typeof args !== "object") {
    throw new Error("RalplanOccSource: args is required.");
  }
  if (typeof args.idea !== "string" || args.idea.trim() === "") {
    throw new Error("RalplanOccSource: args.idea is required and must be a non-empty string.");
  }
  if (typeof args.workingDir !== "string" || args.workingDir === "") {
    throw new Error("RalplanOccSource: args.workingDir is required.");
  }
  ...
  if (!isAbs) {
    throw new Error("RalplanOccSource: args.workingDir must be absolute, got: " + args.workingDir);
  }
  if (args.specPath != null && typeof args.specPath !== "string") {
    throw new Error("RalplanOccSource: args.specPath must be a string when provided.");
  }
  if (args.specPath === "") {
    throw new Error("RalplanOccSource: args.specPath is empty.");
  }
}
...
function analystPromptBuilder(idea, specPath) {
  return (
    ...
    "\nWrite the spec to: " +
    specPath +
    "\nWrite the analyst review to the response. Include a ### Open Questions section at the end.\n" +
...
// ─────────────────────────────────────────────────────────────────────────────
// Budget-aware effective maxIterations
// ─────────────────────────────────────────────────────────────────────────────

function resolveEffectiveMax(maxIterations) {
  // budget is an optional injected helper; if available and has remaining tokens,
  // we cap iterations to stay within a conservative estimate (~3k tokens/iteration).
  const raw = typeof budget === "function" ? budget() : null;
  if (raw && typeof raw.remaining === "number") {
    const slots = Math.floor(raw.remaining / 3000);
    return Math.min(maxIterations, Math.max(1, slots));
  }
  return maxIterations;
}

// ─────────────────────────────────────────────────────────────────────────────
// Main — Planner / Architect / Critic loop with consecutive-acceptance gating
// ─────────────────────────────────────────────────────────────────────────────
...
// OCC artifact path convention: .omc/plans/ and .omc/drafts/
...
const maxIterations = resolveEffectiveMax(baseMaxIterations);
...
if (!args.specPath) {
  ...
  if (analystOut == null) {
    throw new Error(
      "RalplanOccSource: Analyst agent returned null at iteration 1",
    );
  }
}

// Sequential Planner -> Architect -> Critic loop.
// consecutiveAccepts tracks how many consecutive Critic ACCEPT/ACCEPT-WITH-RESERVATIONS
// verdicts have been seen. Consensus requires consecutiveAccepts >= 2.
// Any REVISE/REJECT resets the counter to 0.
const feedback = [];
let iterations = 0;
...
for (; iterations < maxIterations; iterations++) {
  ...
  if (draft == null) {
    throw new Error(
      "RalplanOccSource: Planner returned null at iteration " + iterNum,
    );
  }
  ...
  if (arch == null) {
    throw new Error(
      "RalplanOccSource: Architect returned null at iteration " + iterNum,
    );
  }
  ...
  if (crit == null) {
    throw new Error(
      "RalplanOccSource: Critic returned null at iteration " + iterNum,
    );
  }
  criticVerdict = parseVerdict(crit, CRIT_VERDICT_RE);

  // Architect must APPROVE before we even consider the Critic verdict.
  if (architectVerdict !== "APPROVE") {
    consecutiveAccepts = 0;
  } else if (
    criticVerdict === "ACCEPT" ||
    criticVerdict === "ACCEPT-WITH-RESERVATIONS"
  ) {
    consecutiveAccepts += 1;
    // Backoff log — surfaces progress without a real delay.
    log("CONSECUTIVE_ACCEPTS_" + consecutiveAccepts);
    if (consecutiveAccepts >= 2) {
      // Both architects approved and Critic has been clean for 2 passes.
      break;
    }
  } else {
    // REVISE or REJECT resets the consecutive streak.
    consecutiveAccepts = 0;
  }
...
// Consensus failure: Architect never approved OR Critic never reached 2 consecutive accepts.
if (
  architectVerdict !== "APPROVE" ||
  consecutiveAccepts < 2
) {
  const err = new Error(
    "RalplanOccSource: failed to reach consensus after " +
      iterations +
      " iteration(s).",
  );
