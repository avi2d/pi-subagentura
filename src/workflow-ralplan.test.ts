...
const RALPLAN_SCRIPT = readFileSync(
  join(import.meta.dirname, "workflows", "ralplan-consensus.js"),
  "utf8",
);
 ...
 * Uses `label` (set by the script: "ralplan-planner-N", "ralplan-architect-N", etc.)
...
function dualApproveRunner(): WorkflowAgentRunner {
  return async ({ label }) => {
    ...
    return ok("**DRAFT_WRITTEN: /tmp/plans/drafts/plan_draft.md**");
  };
}
...
describe("AC-1: save/load/list round-trip", () => {
  it("saves ralplan-consensus via saveWorkflowScript and lists it", () => {
    const dir = mkdtempSync(join(tmpdir(), "ralplan-ac1-"));
    saveWorkflowScript("ralplan-consensus", RALPLAN_SCRIPT, dir);

    const list = listSavedWorkflows(dir);
    expect(list.some((e) => e.name === "ralplan-consensus")).toBe(true);

    const loaded = loadWorkflowScript("ralplan-consensus", dir);
    expect(loaded).toBe(RALPLAN_SCRIPT); // byte-identical
  });

  it("sanitizeWorkflowName does not throw for ralplan-consensus", () => {
    expect(sanitizeWorkflowName("ralplan-consensus")).toBe("ralplan-consensus");
  });
...
describe("AC-5: perpetual-reject exits at maxIterations with structured error", () => {
  it("perpetual-reject exits at maxIterations with structured error exposing .verdicts/.draftPath/.iterations/.feedback", async () => {
    ...
    expect(thrownError.draftPath).toMatch(/\/plans\/drafts\/plan_draft\.md$/);
...
describe("AC-10: saved workflow invokable by name via workflow() composition", () => {
  it("pre-saved workflow callable via workflow(name, args) returns expected shape", async () => {
    ...
    const loadWorkflow = (name: string) => {
      if (name === "ralplan-consensus") {
        return loadWorkflowScript(name, dir);
      }
      return null;
    };

    // Parent workflow that calls ralplan-consensus as a child
    const parentScript = `
export const meta = { name: "parent", description: "calls ralplan" };
const r = await workflow("ralplan-consensus", { idea: "test idea", workingDir: "/tmp" });
return r;
`;
