...
export function registerWorkflowTool(
  ...
  registerToolWithDefaultGuidance(pi, {
    ...
    description: [
      ...
      "                            (filter with Boolean).",
      "                            Defaults to multiplexer process isolation (attachable via",
      "                            herdr, tmux, or zellij); falls back to in-process if no",
  ...
  function buildWorkflowCreationPrompt(task: string): string {
    return [
      ...
      "6. Do not set `isolation` unless the workflow explicitly needs to opt out; workflow agents default to multiplexer process isolation (herdr, tmux, or zellij) and fall back to in-process automatically.",
