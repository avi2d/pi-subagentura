      ...
      thunks.map((t) =>
        Promise.resolve()
          .then(() => {
            ...
            // Pre-abort check: signal fires before invoke must not invoke any agent.
            if (opts.signal?.aborted) throw new Error("Workflow aborted.");
            return t();
          })
          .catch((err) => {
            // Re-throw on abort so Promise.all rejects and the calling workflow
            // sees the cancellation. Without this, agents that aborted in-flight
            // silently land as nulls and the caller cannot tell.
      ...
      items.map(async (item, index) => {
        ...
        try {
          for (const stage of fns) {
            // Pre-abort check before each stage so a mid-flight abort throws and short-circuits.
            if (opts.signal?.aborted) throw new Error("Workflow aborted.");
            acc = await stage(acc, item, index);
          }
          return acc;
        } catch (err) {
          // Re-throw on abort so Promise.all rejects and remaining stages / items are not invoked.
          // Silently nulling here would hide cancellation from the caller and leave downstream
          // stages running.
          if (opts.signal?.aborted) throw err;
          return null;
        }
