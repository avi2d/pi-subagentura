...
/** Resolves to a successful result after ~10ms, or rejects on abort. */
function abortableRunner(delayMs = 10): WorkflowAgentRunner {
  return async ({ signal }) => {
...
describe("abort signal propagation", () => {
  it("parallel() re-throws when the signal aborts mid-flight", async () => {
    ...
    const p = runWorkflow(
      meta +
        `const out = await parallel([() => agent("a"), () => agent("b")]); return out;`,
      { runAgent: abortableRunner(10), signal: ac.signal },
    );
    ...
    const p = runWorkflow(
      meta +
        `const stage = async (prev) => { await agent("s"); return prev; };
         const r = await pipeline([1, 2, 3], stage); return r;`,
      { runAgent: abortableRunner(10), signal: ac.signal },
    );
    setTimeout(() => ac.abort(), 2);
    await expect(p).rejects.toThrow(/abort/i);
  ...
  it("pipeline() pre-aborted (signal fires before invoke) re-throws without running agents", async () => {
    ...
    await expect(
      runWorkflow(
        meta +
          `const stage = async (prev) => prev + 1;
           return await pipeline([1, 2], stage);`,
        { runAgent, signal: ac.signal },
      ),
    ).rejects.toThrow(/abort/i);
    expect(calls).toBe(0);
  ...
  it("non-abort failures in pipeline() are still nulled (back-compat)", async () => {
    ...
    const r = await runWorkflow(
      meta +
        `const stage = async (prev) => { throw new Error("stage fail"); };
         return await pipeline([1, 2, 3], stage);`,
      { runAgent: echoRunner() },
    );
    expect(r.result).toEqual([null, null, null]);
  });
});
