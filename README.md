...
- `workflow` — run an agent-authored JS script that deterministically orchestrates isolated sub-agents (`agent`/`parallel`/`pipeline`); supports background runs and saved workflows
- `get_workflow_status` — poll a background workflow's live progress
- `get_workflow_result` — block until a background workflow finishes and return its result
- `cancel_workflow` — abort a running background workflow
- `save_workflow` / `list_workflows` — persist and list named workflows for re-use and `workflow()` composition
...
- `agent(prompt, opts?)` — spawn one isolated sub-agent. `opts`:
  `{ schema?, label?, phase?, model?, persona?, isolation? }`. Without `schema` returns the final
  text; with a JSON Schema it returns a validated object, or `null` after retries. Returns `null` on
  sub-agent error, so scripts can `.filter(Boolean)`. `isolation: "process"` spawns a tmux/zellij Pi
  process (real process isolation, attachable); it falls back to in-process if no multiplexer is
  available.
...
- `workflow(name, args?)` — run a saved workflow inline (one level deep), sharing the parent's agent
  counter and token budget.
- `phase(title)` / `log(msg)` — progress UI only.
- `args` — the JSON you pass in the tool's `args` param. `budget` — `{ total, spent(), remaining() }`.

**Background runs.** Pass `async: true` to the tool to run a workflow in the background: it returns a
`workflowId` immediately, and you poll `get_workflow_status` / `get_workflow_result` (or
`cancel_workflow`). In-process agents are capped at `min(16, cores−2)` concurrent; process-backed
agents at `min(4, cores−2)`.

**Saved workflows.** `save_workflow({ name, script })` validates and persists a script under
`~/.pi-subagentura/workflows/`. Run it later with `workflow({ name })`, compose it from another
script via `workflow(name, args)`, or list them with `list_workflows`.

Constraints: `Date.now()`, `Math.random()`, and argless `new Date()` **throw** (determinism); more
than 1000 agents or 4096 items per call throws. `meta` must be a pure literal.
...
plus a logged warning, never a silent drop. Process-backed (`isolation: "process"`) agents do not
report token usage, so they are not counted against `budget`.

**Still deferred:** `isolation: "worktree"`, `agentType` routing, and `resumeFromRunId` journaling.
