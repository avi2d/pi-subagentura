...
- Watch a real child Pi session and its tool activity live in herdr, tmux, or zellij
...
| `subagent_interactive`                  | Launch an attachable Pi session in herdr, tmux, or Zellij         |
...
herdr, tmux, or zellij pane: watch it live, focus it from the current mux, attach from
...
Starts a separate interactive `pi` process in a multiplexer pane (herdr, tmux, or zellij) and returns immediately with:

- sub-agent id
- pane id and mux backend (herdr, tmux, or zellij)
...
| `v`                    | For interactive agents, capture a bounded terminal snapshot through the mux backend               |
