...
| `src/multiplexer*.ts`           | Pluggable multiplexer interface + herdr, tmux, and zellij backends. Registry auto-detects available backend at runtime.                                                                                           |
...
src/
  ...
  multiplexer{,-tmux,-zellij,-herdr}.ts # mux backend abstraction (herdr + tmux + zellij)
...
docs/                              # This repo is the source of truth (see README); pi-docs only indexes
```
